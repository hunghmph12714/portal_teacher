...
class PaperController extends Controller
{
    ...
    protected function getPayment(Request $request)
    {
        ...
        if (empty($request->filter)) {
            ...
            $payments = Paper::Select(
                ...
                'description',
                'amount',
                DB::raw("DATE_FORMAT(papers.created_at, '%d/%m/%Y') as time_formated"),
                'papers.created_at',
                'papers.status as status',
                'users.name as uname',
                'papers.address as address',
                'center.name as ctname',
                'center.code as code',
                'center.id as ctid'
            )->where('type', 'payment')
        ...
        foreach ($payments as $key => $p) {
            ...
            $transactions = $p->transactions()->select(
                ...
                'transactions.amount',
                DB::raw("DATE_FORMAT(transactions.time, '%d/%m/%Y') as time_formated"),
                'transactions.time',
                'transactions.content',
                'transactions.created_at',
                'debit_account.id as debit_id',
                'debit_account.level_2 as debit_level_2',
                'debit_account.level_1 as debit_level_1',
                'debit_account.name as debit_name',
                'debit_account.type as debit_type',
                'credit_account.id as credit_id',
    ...
    protected function printPaper($id)
    {
        ...
        $paper = Paper::Where('papers.id', $id)->Select(
            'papers.id as id',
            'receipt_number',
            'payment_number',
            'type',
            ...
            'description',
            'amount',
            'papers.created_at as created_at',
            'papers.status as status',
            'users.name as uname',
            'papers.address as address',
            'center.name as ctname',
    ...
    protected function misaUploadReceiptTM(Request $request)
    {
        ...
        $first_line = [
            'Hiển thị trên sổ', 'Ngày hạch toán (*)', 'Ngày chứng từ (*)', 'Số chứng từ (*)', 'Mã đối tượng', 'Tên đối tượng', 'Địa chỉ',
        ...
        foreach ($receipts as $r) {
            ...
            $transactions = $r->transactions()->select(
                'transactions.id as id',
                'transactions.amount',
                DB::raw("DATE_FORMAT(transactions.time, '%m-%d-%Y') as time_formated"),
                'transactions.time',
                'transactions.content',
                'transactions.created_at',
                'debit_account.id as debit_id',
    ...
    protected function misaUploadReceiptNH(Request $request)
    {
        ...
        fprintf($file, chr(0xEF) . chr(0xBB) . chr(0xBF));
        $first_line = [
            'Hiển thị trên sổ', 'Ngày hạch toán (*)', 'Ngày chứng từ (*)', 'Số chứng từ (*)', 'Mã đối tượng',
    ...
    protected function misaUploadPaymentTM(Request $request)
    {
        ...
        fprintf($file, chr(0xEF) . chr(0xBB) . chr(0xBF));
        $first_line = [
            'Hiển thị trên sổ', 'Ngày hạch toán (*)', 'Ngày chứng từ (*)', 'Số chứng từ (*)', 'Mã đối tượng', 'Tên đối tượng', 'Địa chỉ', 'Nhân viên',
    ...
    protected function misaUploadPaymentNH(Request $request)
    {
        ...
        fprintf($file, chr(0xEF) . chr(0xBB) . chr(0xBF));
        $first_line = [
            'Hiển thị trên sổ', 'Phương thức thanh toán', 'Ngày hạch toán (*)', 'Ngày chứng từ (*)', 'Số chứng từ (*)', 'Tài khoản chi', 'Mở tại NH', 'Nội dung thanh toán',
