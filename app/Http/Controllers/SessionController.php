...
class SessionController extends Controller
{
    ...
    protected function getStudentOfProduct(Request $request){
        ...
        if($session){
            ...
            foreach($students as $key => $s){
                $sc = StudentClass::where('class_id', $event->id)->where('student_id', $s->value)->first();
                if($sc){
                    $students[$key]['code'] = $sc->id;

                }else $students[$key]['code'] = '';
    ...
    protected function editSession(Request $request){
        ...
        if($session){
            ...
            //CHeck student
            $new_students = ($request->students != 'null') ? array_column(json_decode($request->students), 'value') : [];
        ...
        foreach($data as $key => $val){
            ...
            if($studentClass){
                ...
                if($student){
                    ...
                    if(count($val) == 8){                    
                        $ss->btvn_comment = $val[7];
                        $ss->btvn_score = $val[6];
                        $ss->save();
                        $result[] = ['code' => $val[1], 'fullname' => $val[2], 'dob' => $val[4], 'school' => $val[3], 'sbd' => $val[6],'room' => $val[7]];
                    }
                    if(count($val) == 9){
                        $ss->btvn_score = $val[6];
                        $ss->btvn_comment = $val[7];
                        $ss->score = $val[8];
                        $ss->save();
                        $result[] = ['code' => $val[1], 'fullname' => $val[2], 'dob' => $val[4], 'school' => $val[3], 'sbd' => $val[6], 'room' => $val[7], 'score' => $val[8]];
                    }
                    if(count($val) == 10){
                        $ss->btvn_score = $val[6];
                        $ss->btvn_comment = $val[7];
                        ...
                        $ss->max_score = $val[9];
                        $ss->save();
                        $result[] = ['code' => $val[1], 'fullname' => $val[2], 'dob' => $val[4], 'school' => $val[3], 'sbd' => $val[6], 'room' => $val[7], 'score' => $val[8], 'max_score' => $val[9]];
                    }                
                    if(count($val) == 11){
                        $ss->btvn_score = $val[6];
                        $ss->btvn_comment = $val[7];
                        ...
                        $ss->comment = $val[10];
                        $ss->save();
                        $result[] = ['code' => $val[1], 'fullname' => $val[2], 'dob' => $val[4], 'school' => $val[3], 'sbd' => $val[6], 'room' => $val[7], 'score' => $val[8], 'max_score' => $val[9], 'comment' => $val[10]];
                    }  
                }
            }
        }
