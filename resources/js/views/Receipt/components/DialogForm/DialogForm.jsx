...
class DialogForm extends React.Component {
    ...
    UNSAFE_componentWillReceiveProps(nextProps){
        if(nextProps.receipt.length != 0){
            let transactions = nextProps.receipt.transactions.map(t => {
            ...
            this.setState({
                address : nextProps.receipt.address,
                name: nextProps.receipt.name,
    ...
    onSubmitTransaction = (e) => {
        ...
        let data = this.state       
        axios.post(baseUrl + '/receipt/create', data)
            .then(response => {
    ...
    onSubmitEdit = (e) => {
        e.preventDefault();
        let data = this.state
        data.receipt_id = this.props.receipt.id
        axios.post(baseUrl + '/receipt/edit', data)
            .then(response => {
